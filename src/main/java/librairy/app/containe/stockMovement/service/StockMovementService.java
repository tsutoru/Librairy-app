...
public class StockMovementService {
    ...
    public int getStockByBookId(String bookId) {
        ...
        int totalIn = arrivalMovementRepository.findByBookId(bookId)
                .stream().mapToInt(m -> m.getQuantity()).sum()
                + cancelledMovementRepository.findByBookId(bookId)
                .stream().mapToInt(m -> m.getQuantity()).sum();

        // OUT = ventes + réservations
        int totalOut = saleMovementRepository.findByBookId(bookId)
                .stream().mapToInt(m -> m.getQuantity()).sum()
                + reservationMovementRepository.findByBookId(bookId)
                .stream().mapToInt(m -> m.getQuantity()).sum();
